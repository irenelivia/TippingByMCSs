  ...
  use vars
      use tracers
  use params
      use slm_vars, only: slm_printout
  implicit none
  ...
  if(SLM.and.OCEAN) then
   print*, 'SLM cannot be used in pure ocean  mode (OCEAN=.true.). stop...'
   call task_abort()
  endif
  ...
  if(dosurface) then
      ...
      if(dodynamicocean) then
           ...
           print*,'SST homogenized in space =',dossthomo
      end if
      print*,'SLM=',SLM
      if(SLM.and.doradforcing) then
           print*, 'doradforcing cannot be set to T when SLM is used. Stop...'
           call task_abort()
      end if
      if(SLM.and.dosfcforcing) then
           print*, 'dosfcforcing cannot be set to T when SLM is used. Stop...'
           call task_abort()
      end if
      if(SLM.and.doperpetual) then
           print*, 'doperpetual cannot be set to T when SLM is used. Stop...'
           call task_abort()
      end if
      if(SLM.and..not.(doshortwave.and.dolongwave)) then
           print*, 'SLM can be used only when both solar and LW radiation allowed. Stop...'
           call task_abort()
      end if
