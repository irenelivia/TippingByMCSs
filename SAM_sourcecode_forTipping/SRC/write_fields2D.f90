...
nfields = 32
! RF adding 1300 and 3500m planes
nfields = nfields + 2*6
! RF removing 9 fields and adding 3 surface fields
nfields = nfields - 6
...
222    continue
    ...
    if(save2Dbin) then
      ...
      write(46) real(dx,4)
      write(46) real(dy,4)
      write(46) real(float(nstep)*dt/(3600.*24.),4)
      ...
      write(c_dx,'(f12.5)') dx
      write(c_dy,'(f12.5)') dy
      write(c_time,'(f12.5)') nstep*dt/(3600.*24.)
...
! 1300m altitude sampling
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=qt1300_xy(i,j)*coef
       qt1300_xy(i,j) = 0.
     end do
   end do
  name='QT1300'
  long_name='Total Water Fraction at 1300 m'
  units='kg/kg'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
  ...
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=t1300_xy(i,j)*coef
       t1300_xy(i,j) = 0.
     end do
   end do
  name='T1300'
  long_name='Temperature at 1300 m'
  units='K'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
  ...
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=p1300_xy(i,j)*coef
       p1300_xy(i,j) = 0.
     end do
   end do
  name='P1300'
  long_name='Pressure at 1300 m'
  units='mbar'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
  ...
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=u1300_xy(i,j)*coef
       u1300_xy(i,j) = 0.
     end do
   end do
  name='U1300'
  long_name='U at 1300 m'
  units='m/s'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
  ...
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=v1300_xy(i,j)*coef
       v1300_xy(i,j) = 0.
     end do
   end do
  name='V1300'
  long_name='V at 1300 m'
  units='m/s'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
  ...
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=w1300_xy(i,j)*coef
       w1300_xy(i,j) = 0.
     end do
   end do
  name='W1300'
  long_name='W at 1300 m'
  units='m/s'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
...
! 3500m altitude sampling
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=qt3500_xy(i,j)*coef
       qt3500_xy(i,j) = 0.
     end do
   end do
  name='QT3500'
  long_name='Total water fraction at 3500 m'
  units='kg/kg'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
  ...
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=t3500_xy(i,j)*coef
       t3500_xy(i,j) = 0.
     end do
   end do
  name='T3500'
  long_name='Temperature at 3500 m'
  units='K'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
  ...
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=p3500_xy(i,j)*coef
       p3500_xy(i,j) = 0.
     end do
   end do
  name='P3500'
  long_name='Pressure at 3500 m'
  units='mbar'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
  ...
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=u3500_xy(i,j)*coef
       u3500_xy(i,j) = 0.
     end do
   end do
  name='U3500'
  long_name='U at 3500 m'
  units='m/s'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
  ...
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=v3500_xy(i,j)*coef
       v3500_xy(i,j) = 0.
     end do
   end do
  name='V3500'
  long_name='V at 3500 m'
  units='m/s'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
  ...
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=w3500_xy(i,j)*coef
       w3500_xy(i,j) = 0.
     end do
   end do
  name='W3500'
  long_name='W at 3500 m'
  units='m/s'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)

! Sampling the first level
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=wsrfc_xy(i,j)*coef
       wsrfc_xy(i,j) = 0.
     end do
   end do
  name='WSRFC'
  long_name='W at surface'
  units='m/s'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
  ...
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=tsrfc_xy(i,j)*coef
       tsrfc_xy(i,j) = 0.
     end do
   end do
  name='TSRFC'
  long_name='T at atm. first level'
  units='K'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
  ...
  nfields1=nfields1+1
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=qvsrfc_xy(i,j)*coef
       qvsrfc_xy(i,j) = 0.
     end do
   end do
  name='QVSRFC'
  long_name='Water Vapor Fraction at atm. first level'
  units='kg/kg'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
