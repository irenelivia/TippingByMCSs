...
NAMELIST /PARAMETERS/ dodamping, doupperbound, docloud, doprecip, &
		...
		OCEAN,LAND,SFC_FLX_FXD,SFC_TAU_FXD, soil_wetness, &
	        ...
	        doseasons, doperpetual, doradhomo, dosfchomo, doisccp, &
                ...
                depth_slab_ocean, Szero, deltaS, timesimpleocean, timesimpleland, &
                dorescale_lhf, rescaling_coeff, restart_resetsst, &
...
!  Set parameters 
	...
	if(RUN3D.and.YES3D.eq.0) then
	  ...
	  call task_abort()
        ...
        if(docoriolis.and..not.dofplane.or.doradlat) then
          dowally = .true.
          print*,'dowally set to true!'
        endif
