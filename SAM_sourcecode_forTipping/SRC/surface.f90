...
if(.not.SFC_FLX_FXD) then
  ...
  if(OCEAN.or.LAND.and.SLM) then
      ...
      !print*,'before rescaling', fluxbq(1,1)
      if(dorescale_lhf) then
        fluxbq(:,:) = rescaling_coeff*fluxbq(:,:)
        !print*,'after rescaling', fluxbq(1,1)
      end if
...
!print*,'before rescaling', lhf_xy(1,1)
...
!if(dorescale_lhf) then
 !   lhf_xy(:,:) = rescaling_coeff*lhf_xy(:,:)
    !print*,'after rescaling', lhf_xy(1,1)
!end if
