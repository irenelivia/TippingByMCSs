module rad
  ...
  use params, only : ggr, coszrs, cp, ocean,  &
       ...
       solar_constant, zenith_angle, nxco2, notracegases, dogrestart
...
contains 
  ...
  subroutine rad_driver 
	...
	! set trace gas concentrations.  Assumed to be uniform in the horizontal.
      ...
      if(mod(nstep,nstat*(1+nrestart_skip)).eq.0) &
                 call write_rad_backups()
    end if ! if icycle == 1
  ...
  subroutine initialize_radiation
    ...
    if(nrestart.eq.0 .or. (nrestart.eq.2.and.dogrestart) ) then
      qrad    (:, :, :) = 0.
      radlwup(:) = 0.
      radlwdn(:) = 0.
      radswup(:) = 0.
...
999     format(f8.2,12e12.4)
  ...
  subroutine write_rad()
    ...
    if(masterproc) print*,'Writing radiation restart file...'
    ...
    if(restart_sep) then
      ...
      do irank = 0, nsubdomains-1
        ...
        if(irank == rank) then
          ...
          filename = constructRestartFileName(case, caseId, nSubdomains)
          ...
          o3, co2, ch4, n2o, o2, cfc11, cfc12, cfc22, ccl4
          close(56)
          !write(66) nradsteps, qrad , radlwup, radlwdn, radswup, radswdn, &
          !     radqrlw, radqrsw, radqrclw, radqrcsw, &
          !     lwDownSurface, lwDownSurfaceClearSky, lwUpSurface, lwUpSurfaceClearSky, & 
          !     lwUpToa,     lwUpToaClearSky,     &
          !     swDownSurface, swDownSurfaceClearSky, swUpSurface, swUpSurfaceClearSky, & 
...
subroutine write_rad_backups()
      ...
      do irank = 0, nsubdomains-1
        ...
        if(irank == rank) then
          ...
          filename_backup = './RESTART/'//case(1:len_trim(case))//'_'// &
                                   ...
                                   rankchar(5-lenstr(rankchar):4)//'_restart_rad_'// &
                                   nstepchar(11-lenstr(nstepchar):10)//'.bin'
          open(56, file = trim(filename_backup), status='unknown',form='unformatted')
    ...
    if(masterproc) print *,'Saved radiation backup restart file. nstep=',nstep
    call task_barrier()
  end subroutine write_rad_backups
  ! ----------------------------------------------------------------------------
