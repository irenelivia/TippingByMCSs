...
if(mod(nstep,nstatis).eq.0) then
      if(masterproc) print*,'Computing statistics'
      call statistics()
      !if(masterproc) print*,'calling micro_statistics'
      call micro_statistics()
      !if(masterproc) print*,'calling sgs_statistics'
      call sgs_statistics()
      !if(masterproc) print*,'passed stat sampling'
end if
...
if(mod(nstep,nprint).eq.0) then
 ...
 if(dompi) then
   ...
   !print*,'I reached this point in stepout'
   !print*,'buffer, buffer1,5:',buffer, buffer1,5
