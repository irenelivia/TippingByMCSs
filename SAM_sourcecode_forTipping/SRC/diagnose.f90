...
integer i,j,k,kb,kc,k200,k500,k850,k1300,k3500
...
! FIND VERTICAL INDICES OF Z=1300M TO EXTRACT QT THERE
k1300 = 1
do k = 1,nzm
   if(z(k).ge.1300.) then
      k1300 = k
...
do j=1,ny
 do i=1,nx
  ...
  ! 1300 meters velocities, temperature and qt
  qt1300_xy(i,j) = qt1300_xy(i,j) + &
        (qv(i,j,k1300)+qcl(i,j,k1300)+qci(i,j,k1300))*dtfactor
  t1300_xy(i,j) = t1300_xy(i,j) + tabs(i,j,k1300)*dtfactor
  p1300_xy(i,j) = p1300_xy(i,j) + (100.*pres(k1300) + p(i,j,k1300))*dtfactor  
  u1300_xy(i,j) = u1300_xy(i,j) + u(i,j,k1300)*dtfactor
  v1300_xy(i,j) = v1300_xy(i,j) + v(i,j,k1300)*dtfactor
  w1300_xy(i,j) = w1300_xy(i,j) + w(i,j,k1300)*dtfactor

 end do
end do


! FIND VERTICAL INDICES OF Z=3500M TO EXTRACT QT THERE
k3500 = 1
do k = 1,nzm
   if(z(k).ge.3500.) then
      k3500 = k
...
do j=1,ny
 do i=1,nx
  ...
  ! 3500 meters velocities, temperature and qt
  qt3500_xy(i,j) = qt3500_xy(i,j) + &
        (qv(i,j,k3500)+qcl(i,j,k3500)+qci(i,j,k3500))*dtfactor
  t3500_xy(i,j) = t3500_xy(i,j) + tabs(i,j,k3500)*dtfactor
  p3500_xy(i,j) = p3500_xy(i,j) + (100.*pres(k3500) + p(i,j,k3500))*dtfactor  
  u3500_xy(i,j) = u3500_xy(i,j) + u(i,j,k3500)*dtfactor
  v3500_xy(i,j) = v3500_xy(i,j) + v(i,j,k3500)*dtfactor
  w3500_xy(i,j) = w3500_xy(i,j) + w(i,j,k3500)*dtfactor

 end do
end do

! More first level data
do j=1,ny
 do i=1,nx

  qvsrfc_xy(i,j) = qvsrfc_xy(i,j) + qv(i,j,1)*dtfactor
  tsrfc_xy(i,j) = tsrfc_xy(i,j) + tabs(i,j,1)*dtfactor
  wsrfc_xy(i,j) = wsrfc_xy(i,j) + w(i,j,2)*dtfactor
