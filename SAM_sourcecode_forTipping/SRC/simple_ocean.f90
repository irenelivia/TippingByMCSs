...
SUBROUTINE sst_evolve
 ...
 use params, only: cp, pi, lcond, tabs_s, ocean_type, dossthomo, &
                   ...
                   delta_sst_l, delta_sst_r, split_domain, nx_split, SST_l, SST_r
 ...
 if(time.lt.timesimpleland) return   
    ...
    if (ocean_type.eq.4) then
      ...
      if (.not. split_domain) then
          sstxy = sstxy - delta_sst*cos(2*pii*time/86400)
          !  sstxy = sstxy - delta_sst*cos(2*pii*(time+21600)/86400) !with 6h lag for cristian's plumes!no lag: sstxy = sstxy - delta_sst*cos(2*pii*time/86400)
      end if

      ! cycle through the positions within each core
      lx = float(nx_gl)*dx
      do i = 1,nx
         tmpx(i) = float(mod(rank,nsubdomains_x)*nx+i-1)
      end do
      
      pii = atan2(0.d0,-1.d0)
      do j=1,ny
         do i=1,nx
          if (tmpx(i) .le. nx_split) then
             sstxy(i,j) = sstxy(i,j) + (SST_l - tabs_s) - delta_sst_l*cos(2*pii*time/86400)
          else
             sstxy(i,j) = sstxy(i,j) + (SST_r - tabs_s) - delta_sst_r*cos(2*pii*time/86400)
          end if
        end do
      end do
